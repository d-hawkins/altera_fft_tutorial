function fft_analysis_gaussian(fft_fixed, fft_double, Bx, N, M, LF_dB, pdfbase)
% -----------------------------------------------------------------------------
% fft_analysis_gaussian.m
%
% 9/1/2024 D. W. Hawkins (dwh@caltech.edu)
%
% Fixed-point Fast Fourier Transform (FFT) quantization analysis for
% complex-valued Gaussian noise inputs.
%
% -----------------------------------------------------------------------------
...
%
fprintf('\n');
fprintf('Gaussian Noise Analysis\n');
fprintf('-----------------------\n');
...
x_double = 2^(Bx-1)*randn(2*M*N,1)*LF/sqrt(2);
...
% Fixed vs Floating-point FFT Quantization error
Q = X-Y;
...
legend(ph,'Input Gaussian Noise', 'Quantization Noise', ...
...
if (abs(Rqq_max_dB-Q_dB)>10)
	...
	% Plot the maximum value of the quantization noise
	text(-100, y_dB+3, sprintf('%.2f dB', Rqq_max_dB),'Color','r', ...
		'HorizontalAlignment','Center')

	% Input quantization noise floor
	text(+100, y_dB+3, sprintf('%.2f dB', Q_dB),'Color','m', ...
		'HorizontalAlignment','Center')
end
